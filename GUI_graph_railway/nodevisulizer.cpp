...
const int NodeVisulizer::NODE_DIAM = 25;
...
const int NodeVisulizer::NODE_RING_DIAM = 10;
...
//        if(inIt->first == this)
//            break;
//    } || inIt == v->inEdgeList.cend()
    if(outIt == outEdgeList.cend())
        return nullptr;
...
EdgeVisulizer *NodeVisulizer::isConnect(NodeVisulizer *v) const
{
    NodeEdgeArray::const_iterator outIt;
    for(outIt = outEdgeList.cbegin(); outIt != outEdgeList.cend(); ++outIt)
    {
...
void NodeVisulizer::setInsideInfo(const QString &str)
{
    ...
    attachedLabel->setPlainText(informationInside);
    //    update();
}

QString NodeVisulizer::getInsideInfo() const
{
    return informationInside;
}
...
void NodeVisulizer::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    {
        setFlag(ItemIsMovable, true);
        if((graphArea->getState() == MyGraphVisulizer::States::ADD_EDGE )|| (graphArea->getState() == MyGraphVisulizer::States::DEL_EDGE))
        {
            setFlag(ItemIsMovable, false);
    ...
    else if(event->button() == Qt::RightButton && graphArea->getState() == MyGraphVisulizer::States::POINTER)
    {
        qDebug() << "Right Button Clicked" << informationInside;
        for(int i = 0; i < outEdgeList.size(); ++i)
        {
            EdgeVisulizer *tmp = outEdgeList[i].second;
            tmp->detach(this);
            delete tmp;
        }
        emit nodeDestroyed(this, this->informationInside);
        delete this;
        return;
    }
    update();
...
NodeVisulizer::NodeVisulizer(MyGraphVisulizer *graphArea, QGraphicsItem *parent) : QGraphicsObject(parent),
    graphArea(graphArea)
{
    attachedLabel = new QGraphicsTextItem(this);
    attachedLabel->setPos(NODE_TOPLEFT_CORNER - QPoint(10, 20));
