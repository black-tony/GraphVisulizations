...
const int EdgeVisulizer::EDGE_WIDTH = 6;
...
void EdgeVisulizer::detach(NodeVisulizer *requestPoint)
{
    if(source != requestPoint && source != nullptr)
    {
        if(source->removeEdge(dest) == nullptr)
            qDebug() << "DEST_ORDER_DETACH_FAIL";
//        source = nullptr;
    }
    if(dest != requestPoint && dest != nullptr)
    {
        if(dest->removeEdge(source) == nullptr)
            qDebug() << "SOURCE_ORDER_DETACH_FAIL";

//        dest = nullptr;
    }
}

void EdgeVisulizer::setEdgeInfo(const QString &newEdgeInfo)
{
